...
class QueryTestK : AbstractQueryTest() {
    ...
    fun newQueryApi() {
        ...
        val newQuery = box.query(
                (TestEntity_.simpleInt lt 12 or (TestEntity_.simpleLong oneOf longArrayOf(1012)))
                        and (TestEntity_.simpleString eq "Fry")
        ...
        val newQueryOr = box.query(
                // (EQ OR EQ) AND LESS
                (TestEntity_.simpleString eq "Fry" or (TestEntity_.simpleString eq "Sarah"))
        ...
        val newQueryAnd = box.query(
                // EQ OR (EQ AND LESS)
                TestEntity_.simpleString eq "Fry" or
                        (TestEntity_.simpleString eq "Sarah" and (TestEntity_.simpleInt lt 12))
        ).build().find()
    ...
    fun intLessAndGreater() {
        putTestEntitiesScalars()
        val query = box.query(
                TestEntity_.simpleInt gt 2003
                        and (TestEntity_.simpleShort lt 2107)
        ).build()
    ...
    fun or() {
        putTestEntitiesScalars()
        val query = box.query(
                TestEntity_.simpleInt eq 2007 or (TestEntity_.simpleLong eq 3002)
        ).build()
    ...
    fun and() {
        ...
        val query = box.query(
                TestEntity_.simpleInt eq 2006 and (TestEntity_.simpleInt eq 2007) or (TestEntity_.simpleInt eq 2008)
        ).build()
        val entities = query.find()
